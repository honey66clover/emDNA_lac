...
void emDNA_LogFile::log_MinimResults(const MinimizationResults& minim_res) {
	...
	//s << "initial energy contribution: \n" << minim_res.initial_energy_contribs << "\n";
    s << "final energy: " << minim_res._final_E << "\n";
	//s << "final energy contribution: \n" << minim_res.final_energy_contribs << "\n";
	//s << "electrostatic energy: " << minim_res._electro_E << "\n";
...
void emDNA_LogFile::log_EnergyComposition(const MinimizationResults& minim_res)
{
    ...
    s << ">> initial energy contributions details\n";
    for (Size i=0; i<minim_res.initial_energy_contribs.size(); ++i) {
        s << minim_res.initial_energy_contribs[i].first;
        s << " -> ";
        s << minim_res.initial_energy_contribs[i].second;
        s << "\n";
    };
	s << ">> final energy contributions details\n";
    for (Size i=0; i<minim_res.final_energy_contribs.size(); ++i) {
        s << minim_res.final_energy_contribs[i].first;
        s << " -> ";
        s << minim_res.final_energy_contribs[i].second;
        s << "\n";
    };
