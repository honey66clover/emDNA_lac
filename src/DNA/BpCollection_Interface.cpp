...
Real BpCollection_Interface::bp_collection_energy() const {
    ...
    Real Elastic = BpCollectionElasticEnergy::elastic_energy(m_bp_collection,
                                                     m_idx_mgr);
													 
	Real Electrostatic =
		BpCollectionElectrostaticEnergy::electrostatic_energy(m_bp_collection);
		
	Real E = Elastic + Electrostatic;
...
std::vector<EnergyContrib> BpCollection_Interface::energy_contribs() const {
    ...
    std::vector<EnergyContrib> energy_contribution;
	Real Elastic = BpCollectionElasticEnergy::elastic_energy(m_bp_collection,m_idx_mgr);
	Real Electrostatic = BpCollectionElectrostaticEnergy::electrostatic_energy(m_bp_collection);
	energy_contribution.push_back(EnergyContrib("elastic",Elastic));
	energy_contribution.push_back(EnergyContrib("electrostatic",Electrostatic));
	// by default we only have the elastic energy
...
VectorN BpCollection_Interface::bp_collection_inline_free_dofs() const {
    ...
    const std::vector<Size> free_dofs_indices = m_idx_mgr.inline_free_dofs_indices();
